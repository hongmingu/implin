...
def get_file_path_group_celeb_photo(instance, filename):
    ext = filename.split('.')[-1]
    id = instance.group_celeb.uuid
    from django.utils.timezone import now
    now = now()
    now_date = now.strftime('%Y-%m-%d-%H-%M-%S')
    filename = "_%s_%s.%s" % (now_date, uuid.uuid4(), ext)

    return os.path.join('photo/%s' % id, filename)


def get_file_path_solo_celeb_photo(instance, filename):
    ext = filename.split('.')[-1]
    id = instance.solo_celeb.uuid
    from django.utils.timezone import now
    now = now()
    now_date = now.strftime('%Y-%m-%d-%H-%M-%S')
    filename = "_%s_%s.%s" % (now_date, uuid.uuid4(), ext)
